        ...
        _hasRemote (hasRemote)
{
    if (!_spriteSheet.loadFromFile(spritesPath)) {
        ...
        oss << "Error: No se puede cargar" << spritesPath
            << " (Player::Player)";
...
void Player::update ()
{
    ...
    unsigned int pixels = dt.asMilliseconds() / PLAYER_SPEED.asMilliseconds();///Pixeles restantes para caminar
    ...
    while (pixels > 0) {
        ...
        switch (_currDir) {
            ...
            case Dir::DOWN:
                ...
                break;
            default:
                std::cerr << "Direction desconocida(Player::update)" << std::endl;
                break;
        }
...
void Player::center (unsigned int& left)
{
    ...
    switch (_currDir) {
        ...
        case Dir::DOWN:
            ...
            break;
        default:
            std::cerr << "Direction desconocida (Player::center)" << std::endl;
            left = 0;
            return;
            break;
    }
...
void Player::pickBonus ()
{
    ...
    switch (bonus) {
        ...
        case Map::PowerUp::NONE:
            break;
        default:
            std::cerr << "Bonus desconocido (Player::center)" << std::endl;
            break;
    }
}
