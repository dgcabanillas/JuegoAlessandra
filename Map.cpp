        ...
        _bufferTexture()
{
    ...
    ///Coloca los potenciadores en el juego
    ...
    if (!_spriteSheetTerrain.loadFromFile(path)) {
        ...
        oss << "Error: No se puede cargar " << path << " (Map::Map)";
    ...
    ///Asignacion y acualiacion de la textura de fondo
    if (!_bufferTexture.create (TILE_SIZE*_nbCol, TILE_SIZE*_nbLines))
        throw std::runtime_error("Error: No se puede inicializar Map._backgroundTexture");
    updateBufferTexture ();
...
{
    ///Dibujo de fondo precalculado : roca, ladrillo, vacio
    {
    ...
    ///Dibujo de los potenciadores
    ///Solo en terreno vacio
